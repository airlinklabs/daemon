...
function write(level: Level, msg: string, extra?: unknown) {
  ...
  if (Bun.env.AIRLINK_JSON_LOGS === '1') {
    ...
    writeToFile(level, redactSecrets(`[${ts()}] ${level.toUpperCase().padEnd(5)}: ${msg}${extraStr}\n`));
    return;
  }
  ...
  if (isTTY) {
    ...
    process.stdout.write(`${redactSecrets(`${ts()} [${level.toUpperCase().padEnd(5)}] ${msg}${extraStr}`)}\n`);
  }
...
function readMetaFromConfig(): { version: string; codename: string } {
  const candidates = [join(process.cwd(), 'storage', 'config.json'), '/etc/daemon/storage/config.json'];
