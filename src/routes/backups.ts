...
export async function handleContainerBackup(req: Request): Promise<Response> {
  ...
  if (!existsSync(volumePath))
    return apiError("container_not_found", "container volume not found", 404);
...
export async function handleContainerRestore(req: Request): Promise<Response> {
  ...
  if (!existsSync(fullPath))
    return apiError("not_found", "backup file not found", 404);
  ...
  if (typeof body.checksum === "string" && body.checksum.length > 0) {
    try {
      ...
      return apiError(
        "internal_error",
        "failed to verify backup checksum",
        500,
      );
    }
  }
  ...
  try {
    ...
    staging = mkdtempSync(
      resolve(getPaths(config.paths).storageRoot, `restore-${body.id}-`),
    );
      ...
      if (cached) {
        if (cached.configFiles && typeof cached.configFiles === "object") {
          await applyConfigFiles(
            ...
            logger.error(
              `restore: applying config files failed for ${body.id}`,
              err,
            );
          });
        }
        await startContainer(
          ...
          logger.error(
            `backup restored but failed to restart container ${body.id}`,
            err,
          );
        });
      }
...
export async function handleContainerBackupDelete(
  req: Request,
): Promise<Response> {
...
export function handleContainerBackupDownload(req: Request): Response {
  ...
  if (!backupPath || typeof backupPath !== "string")
    return apiError("invalid_request", "backup path is required", 400);
...
export async function handleContainerBackupDownloadToken(
  req: Request,
): Promise<Response> {
  const parsed = await parseJsonBody(
    req,
...
export async function handleContainerBackupUpload(
  req: Request,
): Promise<Response> {
  const params = new URL(req.url).searchParams;
  ...
  if (!backupUuid || typeof backupUuid !== "string")
    return apiError("invalid_request", "backup UUID is required", 400);
  ...
  try {
    ...
    if (!req.body)
      return apiError("invalid_request", "request body is required", 400);
    await Bun.write(backupPath, new Response(req.body));

    const size = statSync(backupPath).size;
    logger.info(
      `backup uploaded: container=${id} uuid=${backupUuid} path=${backupPath} bytes=${size}`,
    );
