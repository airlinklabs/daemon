...
import { MAX_PULL_BYTES } from "../config/limits";
import { FS_PULL_ABORT_MS } from "../config/timeouts";
...
export async function handleFsInfo(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!Array.isArray(contents))
      return apiError("internal_error", "could not list directory", 500);
...
export async function handleDownloadToken(
  ...
  if (!entry)
    return apiError("not_found", "download link is invalid or expired", 404);
...
export async function handleFsPull(req: Request): Promise<Response> {
  ...
  const targetDir =
    path && path.trim() !== "" ? path.trim().replace(/^\/+/, "") : "";
  ...
  try {
    ...
    resolvedTarget =
      resolvedDir === "/" ? volumePath : jailPath(volumePath, resolvedDir);
  ...
  const timeout = setTimeout(() => controller.abort(), FS_PULL_ABORT_MS);
  let total = 0;
  ...
  try {
    ...
    if (contentLength > MAX_PULL_BYTES) {
      return apiError(
        "invalid_request",
        "remote file exceeds the 512MB pull limit",
        413,
      );
    }
...
export async function handleFsCopy(req: Request): Promise<Response> {
  ...
  if (!validatePath(source))
    return apiError("path_traversal", "source escapes container volume", 400);
  if (newPath && !validatePath(newPath))
    return apiError("path_traversal", "invalid target path", 400);

  try {
    const destRelative = newPath?.trim()
      ? newPath.trim()
      : defaultCopyTarget(source);
...
export async function handleFsUpload(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (typeof fileContent === "string" && fileContent.startsWith("data:")) {
      ...
      if (!match?.[1])
        return apiError("invalid_request", "invalid base64 format", 400);
    ...
    logger.info(
      `file uploaded: container=${id} path=${targetPath} bytes=${content.byteLength}`,
    );
    return json({
...
export async function handleFsMkdir(req: Request): Promise<Response> {
  ...
  if (!validatePath(name))
    return apiError("path_traversal", "invalid folder path", 400);
