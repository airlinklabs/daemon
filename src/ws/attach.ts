...
export async function attachToContainer(
  ...
  try {
    ...
    logger.info(`stream attach starting: container=${id}`);
    ...
    logStream.on("end", () => {
      logger.debug(`log stream ended: container=${id}`);
    ...
    ws.data._logCleanup = () => {
      try {
        logger.debug(`log stream cleanup: container=${id}`);
        (logStream as unknown as { destroy(): void }).destroy();
      } catch {}
    };
  } catch (err) {
    logger.warn(
      `stream attach failed: container=${id} error=${(err as Error).message}`,
    );
