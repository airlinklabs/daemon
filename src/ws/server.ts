...
import { WS_AUTH_TIMEOUT_MS } from "../config/timeouts";
import { MAX_WS_CONNECTIONS, MAX_WS_AUTH_ATTEMPTS } from "../config/limits";
...
export function assertAuthed(
  data: WsData,
): asserts data is WsData & { authed: true } {
  if (!data.authed) throw new Error("WebSocket not authenticated");
}

let openWsCount = 0;
const MAX_WS = MAX_WS_CONNECTIONS;
const AUTH_TIMEOUT_MS = WS_AUTH_TIMEOUT_MS;
const MAX_AUTH_ATTEMPTS = MAX_WS_AUTH_ATTEMPTS;
...
export function wsMessage(
  ...
  if (eventName === "auth") {
    ...
    const capResult = verifyCapabilityToken(
      key,
      config.key,
      ws.data.containerId,
      ws.data.route,
    ...
    if (capResult.ok) {
      ...
      if (timingSafeKeyEquals(key, config.key)) {
        ...
        if (ws.data.authFailures >= MAX_AUTH_ATTEMPTS) {
          logger.warn(
            `ws auth failed ${MAX_AUTH_ATTEMPTS} times for ${ws.data.containerId}; closing`,
          );
          ws.send(JSON.stringify({ error: "auth failed" }));
          ws.close(1008, "auth failed");
          return;
        ...
        logger.warn(
          `ws auth rejected for ${ws.data.containerId} (${ws.data.authFailures}/${MAX_AUTH_ATTEMPTS})`,
        );
