...
import { WS_IDLE_TIMEOUT_S } from "./config/timeouts";
import { DEFAULT_HOST, WS_UPGRADE_RATE_LIMIT } from "./config/server";
import {
  checkDocker,
  checkDockerRunning,
  docker,
...
function attemptUpgrade(
  ...
  if (route === "nodestats") {
    ...
    const rlErr = checkRateLimit(effectiveIp, WS_UPGRADE_RATE_LIMIT);
...
async function shutdown(signal: string): Promise<void> {
  ...
  const forceTimer = setTimeout(() => {
    logger.error(
      `graceful shutdown exceeded ${SHUTDOWN_TIMEOUT_MS}ms — force exit`,
    );
    process.exit(1);
  }, SHUTDOWN_TIMEOUT_MS);
  forceTimer.unref();
  ...
  // 5. Clean up PID file
  try {
    const { removePidFile } = await import("./app");
    removePidFile();
    logger.info(`PID file cleaned up ${elapsed()}`);
  } catch {
    /* app module not available or no pid file */
  }
