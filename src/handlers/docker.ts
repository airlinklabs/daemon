...
import {
  ...
  STORAGE_ENFORCE_INTERVAL_MS,
  STOP_GRACEFUL_TIMEOUT_MS,
  STOP_GRACEFUL_POLL_MS,
  STOP_FORCE_TIMEOUT_S,
} from "../config/timeouts";
import { TC_BURST, TC_LATENCY } from "../config/limits";
import { CONSOLE_FIFO_RELATIVE_PATH } from "../config/docker";
  ...
  try {
    ...
    const exec = await container.exec({
      Cmd: [
        "/bin/sh",
        "-c",
        `command -v tc >/dev/null 2>&1 && tc qdisc add dev eth0 root handle 1: tbf rate ${mbps}mbit burst ${TC_BURST} latency ${TC_LATENCY}`,
      ],
      AttachStdout: true,
      AttachStderr: true,
    });
