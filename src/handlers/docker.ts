...
export async function checkDockerRunning(): Promise<void> {
  ...
  const code = await proc.exited;
  if (code !== 0) throw new Error(`${cmd} is not running, start it and try again`);
}
...
export async function getContainerState(
  ...
  try {
    ...
    if (getDockerStatusCode(err) !== 404) logger.warn(`inspect failed for ${id}: ${getErrorMessage(err)}`);
...
export async function startContainer(
  ...
  writeFileSync(join(airlinkdDir, 'init.sh'), initScript, { mode: 0o755, encoding: 'utf8' });
...
export async function stopContainer(id: string, stopCmd?: string): Promise<void> {
  ...
  try {
    ...
    if (status !== 304 && status !== 404) logger.warn(`container.stop() for ${id}: ${getErrorMessage(err)}`);
  }
  try {
...
export async function killContainer(id: string): Promise<void> {
  ...
  try {
    ...
    if (getDockerStatusCode(err) !== 404) throw new Error(`failed to kill container ${id}: ${getErrorMessage(err)}`);
  }
  forgetContainer(id);
  setContainerRunning(id, false);
  await archiveLogHistory(id).catch((err) => {
...
export async function deleteContainer(id: string): Promise<void> {
  try {
    ...
    if (getDockerStatusCode(err) !== 404) throw new Error(`failed to delete container ${id}: ${getErrorMessage(err)}`);
  }
  forgetContainer(id);
}
...
export async function sendCommandToContainer(id: string, command: string): Promise<void> {
  try {
    ...
    try {
      ...
      if (getDockerStatusCode(err) === 404) throw new Error(`container ${id} is not running`);
    ...
    if (!cleanedCommand) throw new Error(`empty command ignored for container ${id}`);
