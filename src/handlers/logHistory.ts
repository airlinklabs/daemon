...
const LOG_BUFFER_TTL_MS = positiveIntEnv(
  "AIRLINK_LOG_BUFFER_TTL_MS",
  10 * 60 * 1000,
); // drop lines older than this
const MAX_LOG_LINE_BYTES = positiveIntEnv(
  "AIRLINK_LOG_LINE_MAX_BYTES",
  32 * 1024,
); // truncate a single line at this
const MAX_PENDING_BYTES = positiveIntEnv(
  "AIRLINK_LOG_PENDING_MAX_BYTES",
  64 * 1024,
); // cap partial-line accumulation
...
export function clearLogBuffer(containerId: string): void {
  ...
  // Stop in-flight docker log stream so the next start begins fresh.
...
// Drains pending writes so callers see the latest lines.
...
// Archives a container's rotated + live log into a timestamped tar.gz.
...
async function subscribeToContainerEvents(): Promise<void> {
  if (!runtimeRef) return;
  // Guard against overlapping reconnect timers.
