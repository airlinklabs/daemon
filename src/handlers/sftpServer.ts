...
import { SFTP_SESSION_CLEANUP_MS } from "../config/timeouts";
...
export async function startNativeSftpServer(): Promise<void> {
  ...
  server = new Server(
    { hostKeys: [priv], banner: "Airlink daemon SFTP" },
    (client: Connection, info) => {
      ...
      client.on("authentication", (ctx) => {
        ...
        const outcome = authenticateSftpSession(
          ctx.username,
          ctx.password ?? "",
        );
        if (!outcome.ok) {
      ...
      client.on("ready", () => {
        ...
        if (!session) {
          logger.warn(
            `SFTP client ready but no authed session bound (ownKeys=${Reflect.ownKeys(client).length})`,
          );
          return;
        }
        ...
        const ttlTimer = setTimeout(() => {
          logger.info(
            `SFTP session expired for ${session.serverId}, closing connection`,
          );
          client.end();
        }, ttl);
        ttlTimer.unref();
  ...
  await new Promise<void>((resolveOk, reject) => {
    ...
    if (srv) {
      ...
      srv.listen(config.sftpPort, DEFAULT_HOST, () => resolveOk());
    } else {
  ...
  started = true;
  logger.info(
    `native SFTP server listening on ${DEFAULT_HOST}:${config.sftpPort}`,
  );
}
