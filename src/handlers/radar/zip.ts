...
import { realpathSync, existsSync, mkdtempSync, rmSync } from 'node:fs';
...
export async function zipScanVolume(id: string, options: ZipOptions = {}): Promise<Buffer> {
  ...
  try {
    for (const { diskPath, archivePath } of filesToZip) {
      ...
      await Bun.spawn(['mkdir', '-p', join(staging, archivePath.split('/').slice(0, -1).join('/') || '.')], { stdout: 'pipe', stderr: 'pipe' }).exited;
      await Bun.spawn(['cp', diskPath, dest], { stdout: 'pipe', stderr: 'pipe' }).exited;
    }

    const proc = Bun.spawn(['zip', '-r', '-6', zipPath, '.'], { cwd: staging, stdout: 'pipe', stderr: 'pipe' });
