...
import {
  RADAR_MAX_CONTENT_READ_BYTES,
  RADAR_MAX_CONTENT_SCAN_FILES,
} from "../../config/limits";
import { RADAR_MAX_CONTENT_SCAN_MS } from "../../config/timeouts";
...
async function scanNameMatches(
  ...
  const files = await Array.fromAsync(
    matcher.scan({ cwd: baseDirectory, dot: true }),
  );
  ...
  for (const file of files) {
    ...
    if (
      contentRe &&
      !(await fileMatchesContent(filePath, fileStats.size, contentRe))
    )
      continue;
...
async function scanContentMatches(
  ...
  if (contentRe === undefined || contentRe === null) {
    ...
    logger.warn(
      `content radar pattern has no usable content regex: ${pattern.pattern}`,
    );
    return matches;
  }
  ...
  for await (const file of matcher.scan({
    cwd: baseDirectory,
    dot: true,
    onlyFiles: true,
  })) {
    if (filesScanned >= RADAR_MAX_CONTENT_SCAN_FILES) {
      logger.warn(
        `radar content scan stopped at file cap (${RADAR_MAX_CONTENT_SCAN_FILES} files) for pattern ${pattern.pattern}`,
      );
      break;
    }
    if (performance.now() - startedAt >= RADAR_MAX_CONTENT_SCAN_MS) {
      logger.warn(
        `radar content scan stopped at time budget (${RADAR_MAX_CONTENT_SCAN_MS}ms) for pattern ${pattern.pattern}`,
      );
      break;
    }
    filesScanned++;
    ...
    if (!(await fileMatchesContent(filePath, fileStats.size, contentRe)))
      continue;
...
async function fileMatchesContent(
  filePath: string,
  fileSize: number,
  re: RegExp,
): Promise<boolean> {
  if (fileSize >= RADAR_MAX_CONTENT_READ_BYTES) return false;
...
function passesSizeGuards(size: number, pattern: RadarPattern): boolean {
  if (pattern.size_less_than !== undefined && size >= pattern.size_less_than)
    return false;
  if (
    pattern.size_greater_than !== undefined &&
    size <= pattern.size_greater_than
  )
    return false;
  return true;
}
...
async function isSafeWithinVolume(
  baseDirectory: string,
  filePath: string,
): Promise<boolean> {
  let fileStat: Stats;
