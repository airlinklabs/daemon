...
const crashCallbacks = new Map<
  string,
  (id: string, exitCode: number) => void
>();
...
export function onContainerCrash(
  id: string,
...
type Runtime = {
  listContainers: (opts?: {
    all?: boolean;
  }) => Promise<Array<{ Id: string; State: string; Names?: string[] }>>;
  ...
  const subscribe = async (): Promise<void> => {
    try {
        ...
        try {
          ...
          if (event.Action === "die" && crashCallbacks.has(event.id)) {
            ...
            const exitCode = Number(
              event.Actor?.Attributes?.exitCode ?? event.status ?? "1",
            );
            if (exitCode !== 0) {
              const cb = crashCallbacks.get(event.id);
              if (cb) {
                logger.warn(
                  `container ${event.id} crashed with exit code ${exitCode}`,
                );
          ...
          logger.debug(
            `dropped malformed docker event: ${getErrorMessage(err)}`,
          );
        }
      });
      ...
      stream.on("error", (err: Error) => {
        logger.error(
          "docker event stream had a bad time, reconnecting in 5s",
          err,
        );
