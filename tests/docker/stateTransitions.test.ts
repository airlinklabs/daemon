...
describe("buildHostConfig (resource limits reach HostConfig)", () => {
  const base = {
    ...
    portBindings: { "25565/tcp": [{ HostPort: "25565" }] } as {
      "25565/tcp": [{ HostPort: string }];
    },
    Memory: 2048,
    Cpu: 150,
    Storage: 10240,
  ...
  test("maps Swap semantics (0 → equal to Memory, -1 → unlimited, >0 → memory+swap)", () => {
    ...
    expect(buildHostConfig({ ...base, Swap: 0 }).MemorySwap).toBe(
      2356 * 1024 * 1024,
    );
    expect(buildHostConfig({ ...base, Swap: -1 }).MemorySwap).toBe(-1);
    expect(buildHostConfig({ ...base, Swap: 512 }).MemorySwap).toBe(
      (2356 + 512) * 1024 * 1024,
    );
  });
  ...
  test("port bindings reach HostConfig", () => {
    const hc = buildHostConfig(base) as {
  ...
  test("StorageOpt is set only for the docker runtime, never podman", () => {
    ...
    expect(
      buildHostConfig({ ...base, runtimeName: "podman" }).StorageOpt,
    ).toBeUndefined();
  ...
  test("NET_ADMIN CapAdd granted only when throttling on the docker runtime", () => {
    const hc = buildHostConfig({ ...base, networkRateMbps: 100 }) as {
      CapAdd?: string[];
    };
    expect(hc.CapAdd).toContain("NET_ADMIN");
    expect(
      buildHostConfig({ ...base, networkRateMbps: 0 }).CapAdd,
    ).toBeUndefined();
    expect(
      buildHostConfig({ ...base, networkRateMbps: 100, runtimeName: "podman" })
        .CapAdd,
    ).toBeUndefined();
  });
});
...
describe("parsePortBindings", () => {
  ...
  test("drops malformed entries without throwing", () => {
    const { portBindings } = parsePortBindings(
      "25565,no-split-here,nan:25565,25565:abc,::bad",
    );
