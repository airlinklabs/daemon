...
import {
  ...
  detectCgroupVersion,
} from "../../src/handlers/containerRuntime";
...
describe("MockContainerRuntime capabilities", () => {
  ...
  test("all operations are enabled", () => {
    ...
    for (const [_key, val] of Object.entries(ops)) {
...
describe("MockPodmanRuntime", () => {
  ...
  test("rootful createContainer keeps NET_ADMIN", async () => {
    ...
    expect(runtime.lastCreateOpts.HostConfig.CapAdd).toEqual([
      "NET_ADMIN",
      "SYS_ADMIN",
    ]);
  });
  ...
  test("socket path differs by rootless mode", () => {
    expect(new MockPodmanRuntime(false).socketPath).toBe(
      "/run/podman/podman.sock",
    );
    expect(new MockPodmanRuntime(true).socketPath).toBe(
      "/run/user/1000/podman/podman.sock",
    );
  });

  test("storage driver is overlay", () => {
    expect(new MockPodmanRuntime().capabilities().storageDriver).toBe(
      "overlay",
    );
  });
});
