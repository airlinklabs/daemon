...
describe("appendChunk", () => {
  ...
  test("assembles out-of-order chunks into the correct file (B-3)", async () => {
    ...
    await Promise.all([
      appendChunk(TEST_ID, "ordered.txt", Buffer.from(chunks[2]!), {
        chunkIndex: 2,
        totalChunks: 4,
      }),
      appendChunk(TEST_ID, "ordered.txt", Buffer.from(chunks[0]!), {
        chunkIndex: 0,
        totalChunks: 4,
      }),
      appendChunk(TEST_ID, "ordered.txt", Buffer.from(chunks[3]!), {
        chunkIndex: 3,
        totalChunks: 4,
      }),
      appendChunk(TEST_ID, "ordered.txt", Buffer.from(chunks[1]!), {
        chunkIndex: 1,
        totalChunks: 4,
      }),
    ]);
    expect(readFileSync(fileAt("ordered.txt"), "utf8")).toBe(
      "one-two-three-four",
    );
  ...
  test("concurrent multi-chunk uploads of different files do not interleave", async () => {
    ...
    await Promise.all([
      ...a.map((c, i) =>
        appendChunk(TEST_ID, "a.txt", Buffer.from(c), {
          chunkIndex: i,
          totalChunks: 3,
        }),
      ),
      ...b.map((c, i) =>
        appendChunk(TEST_ID, "b.txt", Buffer.from(c), {
          chunkIndex: i,
          totalChunks: 3,
        }),
      ),
  ...
  test("rejects a chunk index outside the declared total", async () => {
    await expect(
      appendChunk(TEST_ID, "bad.txt", Buffer.from("x"), {
        chunkIndex: 9,
        totalChunks: 2,
      }),
    ).rejects.toThrow();
