...
beforeAll(() => {
  ...
  writeFileSync(
    join(volumeDir, "plugins", "backdoor.php"),
    '<?php eval($_GET["x"]); ?>',
  );
  writeFileSync(
    join(volumeDir, "plugins", "DangerPlugin.jar"),
    "not real content",
  );
  writeFileSync(
    join(volumeDir, "secret.txt"),
    "api_key = sk_live_abcdef123456",
  );
  writeFileSync(
    join(volumeDir, "config.yml"),
    "database:\n  password: hunter2\n",
  );
  writeFileSync(
    join(volumeDir, "small-match.txt"),
    "leak_marker inside a small file",
  );
  ...
  writeFileSync(
    join(volumeDir, "guard-big.txt"),
    `${"x".repeat(500)}GUARDED_MARKER`,
  );
...
describe("radar content scanning", () => {
  test("a content pattern reports a small text file it matches", async () => {
    const results = await scanVolume(ID, script(contentPattern("api_key")), {
  ...
  test("a content pattern with no match reports nothing", async () => {
    const results = await scanVolume(
      ID,
      script(contentPattern("no_such_string_anywhere")),
      {
        baseDirectory: volumeDir,
      },
    );
  ...
  test("size_less_than excludes larger files from content scanning", async () => {
    const results = await scanVolume(
      ID,
      script(contentPattern("guarded_marker", { size_less_than: 100 })),
      {
        baseDirectory: volumeDir,
      },
    );
  ...
  test("a file over the content-read cap is not read or matched", async () => {
    const results = await scanVolume(
      ID,
      script(contentPattern("leak_marker")),
      { baseDirectory: volumeDir },
    );
  ...
  test("filename and extension patterns still work", async () => {
    ...
    expect(matchPaths(results)).toEqual([
      "plugins/backdoor.php",
      "plugins/backdoor.php",
    ]);
  });
  ...
  test("symlinks are not followed out of the volume", async () => {
    const results = await scanVolume(
      ID,
      script(contentPattern("exfil_marker")),
      { baseDirectory: volumeDir },
    );
  ...
  test("a symlinked directory is not traversed", async () => {
    const results = await scanVolume(
      ID,
      script(contentPattern("exfil_dir_marker")),
      { baseDirectory: volumeDir },
    );
