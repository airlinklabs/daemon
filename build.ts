// bun build --compile produces a single binary with the runtime baked in
// run with: bun run build.ts
//
// target strings from bun's docs: https://bun.sh/docs/bundler/executables
// baseline variants are for older CPUs that don't support AVX2 (e.g. some VPS hosts)
...
await Bun.spawn(['mkdir', '-p', 'dist'], { stdout: 'inherit', stderr: 'inherit' }).exited;
